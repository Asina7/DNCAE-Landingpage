...
// Check connection
   ...
   $varFName=trim($_POST["fname"]);
    ...
    $varMessage=trim($_POST["services"]);
    ...
    if ( 0 == count($errors))
    {
        ...
        $mailHeaders .= 'X-Mailer: PHP/' . phpversion();
        	$mailMessage='<BR>First Name: '.$varFName.'<BR>Last Name: '.$var.'<BR>Email: '.$varEmail.'<BR>Phone: '.$varPhone.'<BR>Company: '.$varCompany'<BR>Position: '.$varPosition'<BR>Services: '.$varServices ;
        ...
        if(@mail($toMain, $subject.$DateTime, $mailMessage, $mailHeaders,"-fcontact@wisoftsolutions.com"))
        {
			...
			VALUES ('".addslashes($varFName)."', '".addslashes($varLName)."', '".$varEmail."', '".$varPhone."','".$varCountry."','".$varCompany."','".$varServices."','".$varPosition."','".$varMessage."', '".$ip."', '".$varPage."','".$varSource."','".$DateTime."','".$varCampaign."','".$varMedium."','".$varTerm."')";
